...
const DEFAULT_TOAST_SIDE_OFFSET = 4;
...
type ToastInstance = ReturnType<typeof Toast.useToastManager>['toasts'][number];

function getToastIcon(type?: ToastInstance['type']) {
  if (!type) {
    return null;
  }

  return TOAST_ICONS[type as keyof typeof TOAST_ICONS] ?? null;
}
...
function getSwipeDirections(
  ...
  if (position.includes('center')) {
    return [isTop ? 'up' : 'down'] as const;
  }

  if (position.includes('left')) {
    return ['left', isTop ? 'up' : 'down'] as const;
  }

  return ['right', isTop ? 'up' : 'down'] as const;
}
...
function AnchoredToastItem({ toast }: { toast: ToastInstance }) {
  const Icon = getToastIcon(toast.type);
  const tooltipStyle =
  ...
  return (
    <Toast.Positioner
      ...
      sideOffset={positionerProps.sideOffset ?? DEFAULT_TOAST_SIDE_OFFSET}
      toast={toast}
    >
      <Toast.Root
        className={cn(
